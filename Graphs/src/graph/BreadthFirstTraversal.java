...
public class BreadthFirstTraversal<T> extends AdjacencyGraph<T> implements Traversal<T> {
	 ...
	 */
	private boolean isOnVisitList(T node) 
	{
		return
	...
	private T getUnvisitedNode() {
		for (T node: getNodes()) { // check all the nodes
			if (!isOnVisitList(node)) { // if this node has not been "visited"
	...
	private void visitNode(T node) throws GraphError {
		if (isOnVisitList(node)) return; // if the node is already visited do nothing
		traversal.add(node); // add the node to the traversal
		for (T neighbour: getNeighbours(node)) { // for all this node's neighbours
			if (!isOnVisitList(neighbour)) // if the neighbour hasn't been visited
